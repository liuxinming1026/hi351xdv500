    ...
    ot_sample_svp_coords_info *coords_info)
{
    ...
    td_u32 idx_start, idx_end;
    ot_vgs_line vgs_line[SAMPLE_SVP_NPU_JOINT_COORDS_NUM];
    td_u32 line[SAMPLE_SVP_NPU_JOINT_COORDS_NUM][2] = {
        {1, 3}, {1, 0}, {2, 4}, {2, 0}, {0, 5}, {0, 6}, {5, 7}, {7, 9}, {6, 8}, {8, 10}, {5, 11}, {6, 12}, {11, 12},
        {11, 13}, {13, 15}, {12, 14}, {14, 16}
    };
    td_u32 color[SAMPLE_SVP_NPU_JOINT_COORDS_NUM] = {
    ...
    sample_svp_check_exps_return(coords_info == TD_NULL, ret, SAMPLE_SVP_ERR_LEVEL_ERROR, "rect can't be null\n");
    ...
    for (i = 0; i < SAMPLE_SVP_NPU_JOINT_COORDS_NUM; i++) {
        idx_start = line[i][0];
        idx_end = line[i][1];
        vgs_line[i].start_point.x = (td_u32)coords_info->coords[idx_start].point[0] & ~1;
        vgs_line[i].start_point.y =  (td_u32)coords_info->coords[idx_start].point[1] & ~1;
        vgs_line[i].end_point.x =  (td_u32)coords_info->coords[idx_end].point[0] & ~1;
        vgs_line[i].end_point.y =  (td_u32)coords_info->coords[idx_end].point[1] & ~1;
        vgs_line[i].thick = SAMPLE_SVP_NPU_LINE_THICK;
        vgs_line[i].color = color[i];
    }
    ret = ss_mpi_vgs_add_draw_line_task(vgs_handle, &vgs_task, vgs_line, SAMPLE_SVP_NPU_JOINT_COORDS_NUM);
    sample_svp_check_exps_goto(ret != TD_SUCCESS, fail, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "ss_mpi_vgs_draw_line_task fail,Error(%#x)\n", ret);
...
static td_s32 sample_svp_npu_get_output_info(const sample_svp_npu_task_info *task, size_t *stride,
    td_float **virt_addr, svp_acl_mdl_io_dims *dim)
{
    svp_acl_error ret;
    ...
    *stride = svp_acl_get_data_buffer_stride(data_buffer) / sizeof(td_float);
    sample_svp_check_exps_return(*stride == 0, TD_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR, "get stride is 0!\n");

    *virt_addr = (td_float *)svp_acl_get_data_buffer_addr(data_buffer);
    sample_svp_check_exps_return(*virt_addr == TD_NULL, TD_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR, "get addr failed!\n");

    ret = svp_acl_mdl_get_output_dims(g_svp_npu_model[task->cfg.model_idx].model_desc, 0, dim);
    sample_svp_check_exps_return(ret != TD_SUCCESS || dim->dim_count != SAMPLE_SVP_NPU_DIM_NUM ||
        dim->dims[1] > UINT_MAX || dim->dims[SAMPLE_SVP_NPU_H_DIM_IDX] > UINT_MAX ||
        dim->dims[SAMPLE_SVP_NPU_W_DIM_IDX] > UINT_MAX, TD_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR, "get dim failed!\n");
    return TD_SUCCESS;
}

static td_s32 sample_svp_npu_get_and_calc_joint_coords(const sample_svp_npu_task_info *task,
    const ot_video_frame_info *proc_frame,
    const ot_video_frame_info *show_frame, ot_sample_svp_coords_info *coords_info)
{
    svp_acl_error ret;
    td_u32 c, h, w, i, j, k;
    size_t stride;
    svp_acl_mdl_io_dims dim;
    td_float *out = TD_NULL;
    td_float diff = 0.25;
    ...
    for (i = 0; i < c; i++) {
        ...
        for (j = 0; j < h; j++) {
            for (k = 0; k < w; k++) {
                td_float temp = out[i * (h * stride) + j * stride + k];
    ...
    td_float affine_zoom = (td_float)proc_frame->video_frame.width / (td_float)w;
    td_float affine_offset = ((td_float)proc_frame->video_frame.height - (h * affine_zoom)) / 2.0;
    td_float multiples_w = (td_float)show_frame->video_frame.width / (td_float)proc_frame->video_frame.width;
    td_float multiples_h = (td_float)show_frame->video_frame.height / (td_float)proc_frame->video_frame.height;
    /* calc diff */
    for (i = 0; i < c; i++) {
        td_s32 px = (td_s32)coords_info->coords[i].point[0];
        td_s32 py = (td_s32)coords_info->coords[i].point[1];
        if (px > 1 && px < (td_s32)w - 1 && py > 1 && py < (td_s32)h - 1) {
    ...
    ot_sample_svp_coords_info *coords_info)
{
    ...
    sample_svp_check_exps_return(show_frame == TD_NULL, TD_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "show_frame is NULL!\n");
    sample_svp_check_exps_return(coords_info == TD_NULL, TD_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "rect_info is NULL!\n");
