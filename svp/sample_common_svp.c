    ...
    td_u16 font_color, td_s32 id)
{
    if (id < SAMPLE_SVP_VGS_DIGIT_NUM) {
        int bit = bitmap_row * SAMPLE_SVP_VGS_OSD_FONT_WIDTH + bitmap_col;
...
static td_void sample_common_svp_config_bit_map(int id_bit_high, int id_bit_low, sample_common_svp_text_bitmap *text)
{
    ...
    if (id_bit_high == 0) {
        for (td_u32 bitmap_row = 0; bitmap_row < SAMPLE_SVP_VGS_OSD_FONT_HEIGHT; ++bitmap_row) {
            for (td_s32 col = 0; col < SAMPLE_SVP_VGS_SINGLE_DIGIT_LEFT_SAPCE; col++) {
            ...
            for (td_s32 col = SAMPLE_SVP_VGS_SINGLE_DIGIT_LEFT_SAPCE + SAMPLE_SVP_VGS_OSD_FONT_WIDTH; col <
    ...
    td_u32 height)
{
    ...
    /* config bitmap */
    int id_bit_low = rect->ids[i] % SAMPLE_SVP_VGS_DIGIT_NUM;
    int id_bit_high = (rect->ids[i] / SAMPLE_SVP_VGS_DIGIT_NUM) % SAMPLE_SVP_VGS_DIGIT_NUM;
...
static td_s32 sample_common_svp_get_def_vo_cfg(sample_vo_cfg *vo_cfg)
{
    ...
    vo_cfg->vo_intf_type      = OT_VO_INTF_BT1120;
...
static td_s32 sample_common_svp_vpss_attach_vb(ot_sample_svp_media_cfg *media_cfg, td_s32 vpss_grp_cnt)
{
    ...
    for (grp = 0; grp < vpss_grp_cnt; grp++) {
        for (chn = 0; chn < (td_s32)media_cfg->chn_num; chn++) {
...
end_0:
    for (grp_tmp = 0; grp_tmp < grp; grp_tmp++) {
        for (chn_tmp = 0; chn_tmp < (td_s32)media_cfg->chn_num; chn_tmp++) {
    ...
    pthread_t *vdec_thread, ot_sample_svp_media_cfg *media_cfg, sample_vo_cfg *vo_cfg)
{
    const td_s32 vpss_grp_cnt = 1;
    /* step  1: Init vb */
    td_s32 ret = sample_common_svp_create_vb(media_cfg);
    sample_svp_check_exps_return(ret != TD_SUCCESS, ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
    ...
    /* step 3: Start vpss */
    if (media_cfg->chn_num > SAMPLE_SVP_SINGLE_FRAME_VPSS_CHN_NUM && vdec_attr->sample_vdec_video.ref_frame_num >
        SAMPLE_SVP_SINGLE_FRAME_VPSS_CHN_NUM) { /* 2: one image + one show frame */
